...
- Remaining operational risk: package availability, Trusted Publishing, and Action runtime support
  are verified for `0.5.13`; each future release must rerun `release-evidence` and reverify Trusted
...
```sh
npm exec --yes --package @0disoft/service-catalog-generator@0.5.13 -- scg --version
```
...
- npm package: `@0disoft/service-catalog-generator@0.5.13`.
- npm integrity:
  `sha512-tJdtiXqxLgUQuDQR8SGCsf1OeFA0FrGuBl5qmAyfqCh6s/WIWrw06IjB++u21Q0HHagA9yN3canDpQmGyrbzvQ==`.
- npm provenance predicate: `https://slsa.dev/provenance/v1`.
- npm signature audit: `invalid: 0`, `missing: 0`.
- GitHub Release: `v0.5.13`.
- Release workflow run: `29152476854`, conclusion `success`.
- Release commit: `e741d729cd7abd0fcfdda2698efcad17109b1b81`.
- Mutable Action tag: `v0` points to the `0.5.13` release commit.
- Published CLI smoke: `npm exec --yes --package @0disoft/service-catalog-generator@0.5.13 -- scg --version` returned `0.5.13`.
- Windows install smoke: clean temporary npm install created `node_modules/.bin/scg.cmd` and returned `0.5.13`.
...
```sh
pnpm run release-evidence -- 0.5.13
```
