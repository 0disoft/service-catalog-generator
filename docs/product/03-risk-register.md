...
- Mitigation: per-manifest byte limits, manifest-count limits, bounded parsing concurrency, bounded
  glob matching, a fixed config bootstrap limit, aggregate input/extension/collection/report
  budgets, object-depth limits, hierarchy-sorted source overlap checks, and summary-only Action
  output. Resource failures discard the partial catalog.
